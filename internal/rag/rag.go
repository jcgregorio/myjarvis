...
// Client talks to the goldmine-prime search-server sidecar over HTTP.
...
func (r *Client) Search(ctx context.Context, collection, query string, limit int) ([]Hit, error) {
...
// Searcher wraps a Client + llm.Client to answer questions using
...
func NewSearcher(rag *Client, llm *llm.Client) *Searcher {
	return &Searcher{rag: rag, llm: llm}
}
...
func reRankWikiHits(hits []Hit) (cleaned []Hit, topWasJunk bool) {
	var primary, secondary []Hit
	for i, h := range hits {
...
func (s *Searcher) reformulateQuery(ctx context.Context, question string, hits []Hit) string {
	var titles []string
	for i, h := range hits {
		if i >= 6 {
			break
...
func (s *Searcher) AnswerFromNotes(ctx context.Context, args string) (string, error) {
	var p searchArgs
...
func (s *Searcher) AnswerFromWikipedia(ctx context.Context, args string) (string, error) {
	var p searchArgs
	if err := json.Unmarshal([]byte(args), &p); err != nil {
		return "", fmt.Errorf("parse args: %w", err)
	}
