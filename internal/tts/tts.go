...
import (
	...
	"github.com/jcgregorio/myjarvis/internal/tts/normalize"
)

// Client speaks to a Wyoming-protocol Piper TTS server.
type Client struct {
	addr string
}
...
func (t *Client) Synthesize(text string) ([]byte, error) {
	text = normalize.ForTTS(text)
