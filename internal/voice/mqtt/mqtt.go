...
// Client subscribes to voice device MQTT topics and routes audio
...
// NewClient connects to the MQTT broker and subscribes to
// jarvis/+/audio_start, jarvis/+/audio, and jarvis/+/audio_stop.
func NewClient(ctx context.Context, brokerURL string, router *audio.Router) (*Client, error) {
	u, err := url.Parse(brokerURL)
	if err != nil {
		return nil, fmt.Errorf("parse broker URL: %w", err)
	}

	v := &Client{router: router}
...
func (v *Client) handleMessage(pr paho.PublishReceived) (bool, error) {
...
func (v *Client) PublishLED(device, state string) {
	if v.cm == nil {
		return
	}
...
// PublishStopPlayback tells the device to stop playing TTS audio.
func (v *Client) PublishStopPlayback(device string) {
	if v.cm == nil {
		return
	}
	topic := fmt.Sprintf("jarvis/%s/stop_playback", device)
	log.Printf("[mqtt] publishing stop_playback → %s", device)
	if _, err := v.cm.Publish(context.Background(), &paho.Publish{
		Topic:   topic,
		QoS:     0,
		Payload: []byte("stop"),
	}); err != nil {
		log.Printf("[mqtt] publish stop_playback error: %v", err)
	}
}

// PublishStopStreaming tells the device to stop streaming audio.
func (v *Client) PublishStopStreaming(device string) {
	if v.cm == nil {
		return
	}
...
func (v *Client) SignalError(device string) {
