package main

import (
	"os"
	"os/exec"
	"path/filepath"
	"testing"
	"time"

	"github.com/stretchr/testify/assert"
)

func TestResolveDate(t *testing.T) {
	today, err := time.Parse("2006-01-05", "2026-05-21")
	assert.NoError(t, err)
	today = today.UTC()
	tests := []struct {
		in   string
		want time.Time
	}{
		{"", today},
		{"today", today},
		{"Today", today},
		{"yesterday", today.AddDate(0, 0, -1)},
		{"3 days ago", today.AddDate(0, 0, -3)},
		{"1 day ago", today.AddDate(0, 0, -1)},
		{"next tuesday", today}, // unrecognized → today
	}
	for _, tt := range tests {
		if got := resolveDate(today, tt.in); got != tt.want {
			t.Errorf("resolveDate(%q) = %q, want %q", tt.in, got, tt.want)
		}
	}
}

func TestAppendUnderLogSection(t *testing.T) {
	dir := t.TempDir()

	// 1. No ## Log section yet → create it at EOF.
	p1 := filepath.Join(dir, "a.md")
	os.WriteFile(p1, []byte("# 5 Myrtle Ct\n\nNice place.\n"), 0644)
	if err := appendUnderLogSection(p1, "2026-05-18, 24, Removed personal items"); err != nil {
		t.Fatal(err)
	}
	want1 := "# 5 Myrtle Ct\n\nNice place.\n\n## Log\n2026-05-18, 24, Removed personal items\n"
	if got, _ := os.ReadFile(p1); string(got) != want1 {
		t.Errorf("create section:\n got %q\nwant %q", got, want1)
	}

	// 2. Append a second entry → goes under the existing ## Log.
	if err := appendUnderLogSection(p1, "2026-05-19, 0, Rick replaced the lock"); err != nil {
		t.Fatal(err)
	}
	want2 := want1 + "2026-05-19, 0, Rick replaced the lock\n"
	if got, _ := os.ReadFile(p1); string(got) != want2 {
		t.Errorf("append existing:\n got %q\nwant %q", got, want2)
	}

	// 3. ## Log followed by another section → insert before the next heading.
	p3 := filepath.Join(dir, "b.md")
	os.WriteFile(p3, []byte("## Log\n2026-01-01, 8, old\n\n## Notes\nkeep me\n"), 0644)
	if err := appendUnderLogSection(p3, "2026-05-18, 8, new"); err != nil {
		t.Fatal(err)
	}
	want3 := "## Log\n2026-01-01, 8, old\n2026-05-18, 8, new\n\n## Notes\nkeep me\n"
	if got, _ := os.ReadFile(p3); string(got) != want3 {
		t.Errorf("insert before next heading:\n got %q\nwant %q", got, want3)
	}
}

func TestResolveProperty(t *testing.T) {
	dir := t.TempDir()
	orig := propertiesDir
	propertiesDir = dir
	defer func() { propertiesDir = orig }()

	for _, n := range []string{
		"5 Myrtle Ct. Ocean Isle Beach 28469",
		"0 Hayes Run Rd. New Hill NC",
	} {
		os.WriteFile(filepath.Join(dir, n+".md"), []byte("x"), 0644)
	}

	cases := []struct {
		in     string
		want   string
		wantOK bool
	}{
		{"5 Myrtle Ct. Ocean Isle Beach 28469", "5 Myrtle Ct. Ocean Isle Beach 28469", true}, // exact
		{"0 hayes run rd. new hill nc", "0 Hayes Run Rd. New Hill NC", true},                 // exact, case-insensitive
		{"myrtle", "5 Myrtle Ct. Ocean Isle Beach 28469", true},                              // unambiguous substring
		{"hayes run", "0 Hayes Run Rd. New Hill NC", true},                                   // unambiguous substring
		{"n", "", false},          // ambiguous (both contain) → fail gracefully
		{"WindJammer", "", false}, // unknown → fail gracefully
		{"", "", false},
	}
	for _, c := range cases {
		got, ok := resolveProperty(c.in)
		if got != c.want || ok != c.wantOK {
			t.Errorf("resolveProperty(%q) = (%q, %v), want (%q, %v)", c.in, got, ok, c.want, c.wantOK)
		}
	}
}

func Test_dateToTTS(t *testing.T) {
	today, err := time.Parse("2006-01-02", "2026-05-21")
	assert.NoError(t, err)
	assert.Equal(t, "May 21", dateToTTS(today))
}

func Test_dateToMarkdown(t *testing.T) {
	today, err := time.Parse("2006-01-02", "2026-05-21")
	assert.NoError(t, err)
	assert.Equal(t,
		"2026-05-21",
		dateToMarkdown(today),
	)
}

func TestShortProp(t *testing.T) {
	tests := []struct{ in, want string }{
		{"5 Myrtle Ct. Ocean Isle Beach 28469", "5 Myrtle Ct"},
		{"0 Hayes Run Rd. New Hill NC", "0 Hayes Run Rd"},
		{"Simple Name", "Simple Name"}, // no separator → whole string
		{"Foo, Bar", "Foo"},             // comma separator
		{"", ""},
		{".leadingDot", ".leadingDot"}, // separator at index 0 → whole string
		{"Trailing dot.", "Trailing dot"},
	}
	for _, tt := range tests {
		assert.Equalf(t, tt.want, shortProp(tt.in), "shortProp(%q)", tt.in)
	}
}

// setupGitObsidian creates a temp obsidian-style repo with a bare remote
// and overrides obsidianRepo + propertiesDir so LogPropertyEvent's git
// pull/commit/push actually work in tests. The var overrides are
// restored via t.Cleanup. Returns the repo's path.
func setupGitObsidian(t *testing.T) string {
	t.Helper()
	base := t.TempDir()
	remote := filepath.Join(base, "remote.git")
	repo := filepath.Join(base, "repo")

	run := func(dir, name string, args ...string) {
		cmd := exec.Command(name, args...)
		if dir != "" {
			cmd.Dir = dir
		}
		if out, err := cmd.CombinedOutput(); err != nil {
			t.Fatalf("%s %v: %v\n%s", name, args, err, out)
		}
	}
	run("", "git", "init", "--bare", remote)
	run("", "git", "clone", remote, repo)
	run(repo, "git", "config", "user.email", "test@example.com")
	run(repo, "git", "config", "user.name", "Test")
	if err := os.MkdirAll(filepath.Join(repo, "Properties"), 0o755); err != nil {
		t.Fatal(err)
	}
	// initial empty commit so a branch exists to push and later pull
	run(repo, "git", "commit", "--allow-empty", "-m", "init")
	run(repo, "git", "push", "-u", "origin", "HEAD")

	origRepo, origProps := obsidianRepo, propertiesDir
	obsidianRepo = repo
	propertiesDir = filepath.Join(repo, "Properties")
	t.Cleanup(func() {
		obsidianRepo = origRepo
		propertiesDir = origProps
	})
	return repo
}

func TestLogPropertyEvent_EmptyDescription(t *testing.T) {
	// Whitespace-only description is rejected before any property
	// resolution, file I/O, or git.
	resp, err := LogPropertyEvent("anything", 0, "   ", "today")
	assert.Error(t, err)
	assert.Empty(t, resp)
}

func TestLogPropertyEvent_UnknownProperty(t *testing.T) {
	// resolveProperty fails before any git call — verify graceful
	// spoken message and nil error (decision: don't auto-create).
	tmp := t.TempDir()
	orig := propertiesDir
	propertiesDir = tmp
	t.Cleanup(func() { propertiesDir = orig })

	resp, err := LogPropertyEvent("WindJammer", 0, "Rick replaced the lock", "today")
	assert.NoError(t, err)
	assert.Equal(t, "I don't have a property page for WindJammer.", resp)
}

func TestLogPropertyEvent_HappyPath(t *testing.T) {
	repo := setupGitObsidian(t)
	page := "5 Myrtle Ct. Ocean Isle Beach 28469"
	pagePath := filepath.Join(repo, "Properties", page+".md")
	if err := os.WriteFile(pagePath, []byte("# 5 Myrtle Ct\n"), 0o644); err != nil {
		t.Fatal(err)
	}

	resp, err := LogPropertyEvent(page, 24, "Removed personal items", "today")
	assert.NoError(t, err)
	// Spoken confirmation should include short prop, hours unit (plural), description.
	assert.Contains(t, resp, "5 Myrtle Ct")
	assert.Contains(t, resp, "24 hours")
	assert.Contains(t, resp, "Removed personal items")

	content, err := os.ReadFile(pagePath)
	assert.NoError(t, err)
	assert.Contains(t, string(content), "## Log")
	assert.Contains(t, string(content), ", 24, Removed personal items")
}

func TestLogPropertyEvent_NegativeHoursClamped(t *testing.T) {
	repo := setupGitObsidian(t)
	page := "0 Hayes Run Rd. New Hill NC"
	pagePath := filepath.Join(repo, "Properties", page+".md")
	if err := os.WriteFile(pagePath, []byte("# Hayes Run\n"), 0o644); err != nil {
		t.Fatal(err)
	}

	resp, err := LogPropertyEvent(page, -5, "Rick replaced the lock", "today")
	assert.NoError(t, err)
	assert.Contains(t, resp, "0 hours")

	content, _ := os.ReadFile(pagePath)
	assert.Contains(t, string(content), ", 0, Rick replaced the lock")
	assert.NotContains(t, string(content), ", -5,")
}

func TestLogPropertyEvent_OneHourSingular(t *testing.T) {
	repo := setupGitObsidian(t)
	page := "5 Myrtle Ct. Ocean Isle Beach 28469"
	pagePath := filepath.Join(repo, "Properties", page+".md")
	if err := os.WriteFile(pagePath, []byte("# Myrtle\n"), 0o644); err != nil {
		t.Fatal(err)
	}

	resp, err := LogPropertyEvent(page, 1, "Walkthrough", "today")
	assert.NoError(t, err)
	// "1 hour" singular, not "1 hours". Match with a trailing ", " so the
	// substring check can't accidentally hit "1 hours".
	assert.Contains(t, resp, "1 hour, ")
	assert.NotContains(t, resp, "1 hours")
}
