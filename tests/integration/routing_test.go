...
func fixtureEntities() []ha.Entity {
	mk := func(id, name string) ha.Entity {
		return ha.Entity{EntityID: id, Attributes: map[string]any{"friendly_name": name}}
	}
	return []ha.Entity{
...
func TestRouting(t *testing.T) {
	...
	tools := llm.BuildTools(fixtureEntities(), fixtureLists, fixtureProperties)
	...
	// calls below are warm and comparable.
	{
		...
		_, _, _ = lc.Chat(wctx, decorate("turn on the kitchen light"), tools)
		wcancel()
	}
	...
	for _, c := range routeCases {
		...
		t.Run(c.category+"/"+c.prompt, func(t *testing.T) {
			...
			start := time.Now()
			toolCalls, reply, err := lc.Chat(ctx, decorate(c.prompt), tools)
